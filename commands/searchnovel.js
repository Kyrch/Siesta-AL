        ...
        message.reply('Loading...').then(async (resultMessage) => {
            ...
            array.forEach(async (e) => {
                ...
                let info = {
                    ...
                    volumes: firstResult[i].info.volumes == null ? 'Undefined' : firstResult[i].info.volumes.toString(),
            ...
            lisN(resultMessage, message, pe, 1, Discord, client)
            list.clear()
        })
    }
}
...
lisN = async (listMsgN, message, pe, page, Discord, client) => {
    ...
    if (listMsgN) await listMsgN.edit({
        embeds: [embed],
        components: [row]
    });
    else listMsgN = await message.channel.send({
        embeds: [embed],
        components: [row]
    });
    ...
    if (client != undefined) {
        client.on('interactionCreate', interaction => {
            ...
            if (interaction.customId == 'back') {
                if (page == 1) lisN(listMsgN, message, pe, pe.length, Discord)
                else lisN(listMsgN, message, pe, page - 1, Discord)
                page--
            }

            if (interaction.customId == 'next') {
                if (page == pe.length) lisN(listMsgN, message, pe, 1, Discord)
                else lisN(listMsgN, message, pe, page + 1, Discord)
                page++
            }

            if (interaction.customId == 'fback') {
                lisN(listMsgN, message, pe, 1, Discord)
                page = 1
            }

            if (interaction.customId == 'fnext') {
                lisN(listMsgN, message, pe, pe.length, Discord)
                page = pe.length
            }
        });
    }
